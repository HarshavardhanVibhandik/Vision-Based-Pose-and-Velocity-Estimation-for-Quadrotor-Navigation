...
for n = 2:length(sampledData)
    ...
    %prev_Image = insertMarker(prev_image,prev_corners.Location,'+','Color','red');
    ...
    %Using dot product of z-axis of Rc2w 
    %a.b = a*b * cos(theta)
...
%   Hypotenuse = Z
    ...
    H =[A,B];
    Velocity_c_world_camera = pinv(H) * p_dot;
    ...
    adjoint= [R_c2w  -R_c2w*skew_camera_body ; 
                zeros(3,3) R_c2w] ;
    Velocity = adjoint * Velocity_c_world_camera;
    
    %% RANSAC    
    % Write your own RANSAC implementation in the file velocityRANSAC
    [Velocity] = velocityRANSAC(prev_corners,curr_points(:,1:2),Z,R_c2w,0.2,dt);
